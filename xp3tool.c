...
static void decompress_data(uint64_t data_offset, wchar_t* out_file_path)
{
...
static int unpack(const wchar_t* file_path, const wchar_t* output_dir)
{
	...
	decompress_data(hdr.index_offset + sizeof(xp3_index) + 1, out_file_path);
	...
	for (int i = 0; i < idx_file_size;)
	{
		...
		{
			// XXX: Skip the first iteration for Morenatsu XP3, as the filename apparently contains
			// invalid path name chars (on purpose? I might be missing something)
			if (first)
			{
			...
			parse_segment(&segments[j], idx_data + i, j * sizeof(xp3_segment));
			i += sizeof(xp3_segment);
